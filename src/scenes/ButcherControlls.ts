...
export class ButcherControlls extends Controlls {
    ...
    this.circleSearchCharacterEvent = new Phaser.Time.TimerEvent({
      ...
      callback: () => {
        ...
        if (bodies.length > 0) {
          ...
          this.character.sprite.scene.time.removeEvent(
  ...
  update(delta: number) {
    if (this.chasePoint) {
      ...
      if (Math.abs(dirX) > 10) {
        this.character.sprite.setVelocityX(
          (Math.abs(dirX) / dirX) * runningSpeedScale,
        );
      } else {
      ...
      if (Math.abs(dirY) > 10) {
        this.character.sprite.setVelocityY(
          (Math.abs(dirY) / dirY) * runningSpeedScale,
        );
      } else {
