...
export class CharacterWithGoToScheduledPointState extends CharacterState {
  ...
  start() {
    ...
    this.fetchFollowPathEvent = new Phaser.Time.TimerEvent({
      ...
      loop: true,
      callback: () => {
        const scene = this.character.sprite
          .scene as unknown as GameSceneTopPossibilities;
        const newPath = scene.onRequestCharacterFollowPath(
    ...
    if (this.currentPointIndex >= this.autoFollowPathPoints.length) {
      if (this.followingCharacter !== -1) {
        ...
        if (
          ...
          this.schedulePoints.originalScheduleForWalking.length
        ) {
      ...
      this.character.sprite.emit(
        sceneEventConstants.chase,
        true,
        point.x,
        point.y,
      );
