...
export class PlatformerInputAgent implements ScriptComponent {
  ...
  createPathDebugSpline() {
    ...
    for (let i = 0; i < this.currentSelectedPath.length - 1; i++) {
      const line = new Phaser.Geom.Line(
        this.currentSelectedPath[i].x,
        this.currentSelectedPath[i].y,
        this.currentSelectedPath[i + 1].x,
        this.currentSelectedPath[i + 1].y,
      );
  ...
  calculatePathToTarget(): NavigationState {
    ...
    if (this.lastDestination) {
      if (
        this.lastDestination.x === destination.x &&
        this.lastDestination.y === destination.y
      ) {
  ...
  update(delta: number) {
    ...
    if (
      ...
      this.navigationState === NavigationState.NOT_FOUND
    ) {
      return;
    }
