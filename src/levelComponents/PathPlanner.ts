...
export type NavMeshPointMap = Map<PATH_POINT_KEY, PathPoint>;
...
export class PathPlanner {
  ...
  resetPathTable(from: PATH_POINT_KEY) {
    ...
    for (const [key, _node] of this.vertexes) {
      const toNode = this.vertexes.get(key);
      if (!fromNode || !toNode) {
        this.pathTable[key] = new PathTableItem(
        ...
        this.pathTable[key] =
          key === from
            ...
            : new PathTableItem(
                Number.POSITIVE_INFINITY,
                undefined,
                this.calculateHeuristic(fromNode, toNode),
              );
      }
    }
  }
  ...
  extractPath(to: PATH_POINT_KEY): PathPoint[] {
    let currentNode = to;
  ...
  execute(from: PATH_POINT_KEY, to: PATH_POINT_KEY): PathPoint[] {
    const fromNode = this.vertexes.get(from);
    if (!fromNode) {
      return [];
    }
